...
## Bound context
- ticket: GATE1-121
- initiative: INIT-DSOPS-UI-PACKAGE-001
- handoff_path: prd/reports/Impact-Map-INIT-DSOPS-UI-PACKAGE-001.md
- workspace: /workspace/drivestream-meta
- skill_id: resolve-pr-review
...
## Workspace
Root: `/workspace/drivestream-meta`.

## Handoff baton (required)
1. Follow this skill's `SKILL.md`. Persist initiative artifacts under
   `/workspace/drivestream-meta`.
2. Then **overwrite** the file at exactly `prd/reports/Impact-Map-INIT-DSOPS-UI-PACKAGE-001.md` with the map stage
   `handoff:` envelope (plain YAML or a single fenced yaml block).
3. Do not leave `prd/reports/Impact-Map-INIT-DSOPS-UI-PACKAGE-001.md` empty. Do not rely on chat-only handoff for
