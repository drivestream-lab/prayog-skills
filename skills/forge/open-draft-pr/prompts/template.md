# Invoke: open-draft-pr

You are executing the **open-draft-pr** skill (Open or update Draft PR + projection labels from handoff.forge.).
...
2. **Overwrite** the file at exactly `{{handoff_path}}` with the `handoff:`
