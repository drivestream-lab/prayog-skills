...
| P15 | **UI lock** — when the initiative has UI: `LOCKED-{INIT}.md` exists on the spec branch; §0 cites Selected + Improvements + lock path; UI TASKs rebuild that composition (no explore A/B/C leftovers). When non-UI: lock marked N/A with reason (`ui-variations` skipped). **FAIL** if UI in scope and lock missing, or explore variant/preview-switch files remain |
